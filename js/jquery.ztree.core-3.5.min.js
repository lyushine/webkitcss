...
(function($){
	...
	_setting = {
		propurl: "http://ued.ctrip.com/blog/wp-content/webkitcss/prop/",
		treeId: "",
		treeObj: null,
		view: {
			addDiyDom: null,
	...
	data = {
		...
		getNodeName: function(setting, node) {
			...
			if(!node['cname'])
				return "" + node[nameKey];
			else return "" + node['cname'] + '<span class="ename">(' + node[nameKey] + ')<span>';
		},
		getNodeTitle: function(setting, node) {
			var t = setting.data.key.title === "" ? setting.data.key.namesetting.data.key.name : setting.data.key.title;
			return "" + node[t];
	...
	view = {
		...
		makeDOMNodeNameAfter: function(html, setting, node) {
			...
			if(typeof node.values != "undefined")
			{
				...
				for(var i = 0; i < node.values.length; i++){
					values += "<li>" + node.values[i] + "</li>";
					//values =+ ("<li>" + node.values[i] + "<li>");
				}
				values +="</ul>";
			}
			if(typeof settings.contentNodes != "undefined" && typeof values != "undefined"){
				html.push("<h3>语法</h3>" + values);
		...
		makeDOMNodeNameBefore: function(html, setting, node) {
			...
			if(node.tocont != false){
			html.push("<a href='",G.url+"prop/", node.name.replace("-webkit-","") +".html","' id='", node.tId, consts.id.A, "' class='level", node.level,"' treeNode", consts.id.A," ", (node.click || ''),
				"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
				"'");
			}
			else{
				html.push("<a href='","","","' id='", node.tId, consts.id.A, "' class='level", node.level,"' treeNode", consts.id.A," onclick=\"return false", (node.click || ''),
				"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
				"'");
			}
		...
		setNodeName: function(setting, node) {
			...
			nObj.empty();
				nObj.html(data.getNodeName(setting, node));  //lyushine update
